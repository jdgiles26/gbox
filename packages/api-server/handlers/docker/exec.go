...
func handleExecBox(h *DockerBoxHandler, req *restful.Request, resp *restful.Response) {
	...
	logger.Info("Received exec request for box: %s", boxID)
	...
	if container.State != "running" {
		logger.Info("Box %s is not running (current state: stopped)", boxID)
	...
	logger.Debug("Exec request: cmd=%v, args=%v, tty=%v, stdin=%v, stdout=%v, stderr=%v",
	...
	logger.Debug("Accept header: %s", accept)

	// Validate Accept header
	if accept != models.MediaTypeRawStream && accept != models.MediaTypeMultiplexedStream {
		logger.Info("Unsupported Accept header: %s", accept)
...
func (h *DockerBoxHandler) handleCommandExecution(ctx context.Context, containerID string, execReq *models.BoxExecRequest, resp *restful.Response, req *restful.Request) error {
	...
	logger.Debug("Creating exec with config: %+v", execConfig)
	...
	logger.Debug("Created exec instance: %s", execCreate.ID)
	...
	logger.Debug("Attached to exec instance")
	...
	if upgrade == "tcp" && connection == "Upgrade" {
		...
		logger.Debug("Protocol upgrade requested, using raw stream")
	} else {
		// No protocol upgrade requested
		fmt.Fprintf(clientConn, "HTTP/1.1 200 OK\r\n")
		fmt.Fprintf(clientConn, "Content-Type: %s\r\n", models.MediaTypeMultiplexedStream)
		logger.Debug("No protocol upgrade requested, using multiplexed stream")
	}
	...
	if f, ok := clientConn.(http.Flusher); ok {
		f.Flush()
		logger.Debug("Flushed response headers")
	}
	...
	if execReq.TTY || (upgrade == "tcp" && connection == "Upgrade") {
		...
		go func() {
			if _, err := io.Copy(clientConn, execAttach.Reader); err != nil {
				if err != io.EOF && !isConnectionClosed(err) {
					logger.Error("Error copying from container: %v", err)
					stdoutDone <- err
				}
			}
		...
		if execReq.Stdin {
			go func() {
				if _, err := io.Copy(execAttach.Conn, clientConn); err != nil {
					if err != io.EOF && !isConnectionClosed(err) {
						logger.Error("Error copying to container: %v", err)
					}
				}
				...
				if closeWriter, ok := execAttach.Conn.(interface{ CloseWrite() error }); ok {
					if err := closeWriter.CloseWrite(); err != nil {
						logger.Error("Error closing write end: %v", err)
					}
				}
				close(stdinDone)
			}()
		...
		logger.Debug("Starting multiplexed stream")
		go func() {
	...
	select {
	case <-ctx.Done():
		logger.Info("Context cancelled while waiting for stdin")
		execAttach.Close()
		return ctx.Err()
	case <-stdinDone:
		logger.Info("Stdin stream completed")
	}

	// Then wait for stdout/stderr to complete
	select {
	case <-ctx.Done():
		logger.Info("Context cancelled while waiting for stdout")
		execAttach.Close()
		return ctx.Err()
	case err := <-stdoutDone:
		...
		logger.Info("Stdout stream completed")
	}

	// Get the exit status before closing the connection
	execInspect, err := h.client.ContainerExecInspect(ctx, execCreate.ID)
	if err != nil {
		logger.Error("Error inspecting exec: %v", err)
		execAttach.Close()
		return err
	}

	logger.Info("Exec completed with exit code: %d", execInspect.ExitCode)
