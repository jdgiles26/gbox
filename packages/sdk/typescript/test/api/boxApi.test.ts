...
describe('BoxApi', () => {
  ...
  describe('list', () => {
    it('should list boxes without filters', async () => {
      const mockData = {
        boxes: [{ id: 'box1', image: 'img1', status: 'running' } as BoxData],
      };
    ...
    it('should list boxes with label filters', async () => {
      const filters: BoxListFilters = {
        label: ['status=running', 'image=img1'],
      };
      const mockData = {
        boxes: [{ id: 'box1', image: 'img1', status: 'running' } as BoxData],
      };
      ...
      const expectedParams = {
        filter: ['label=status=running', 'label=image=img1'],
      };
      expect(mockRequest).toHaveBeenCalledWith({
        method: 'get',
        url: '/api/v1/boxes',
    ...
    it('should handle array id filters correctly', async () => {
      ...
      const mockData = {
        boxes: [
          { id: 'box1', status: 'stopped' } as BoxData,
          { id: 'box2', status: 'running' } as BoxData,
        ],
      };
      const mockResponse: Partial<AxiosResponse> = { data: mockData };
      mockRequest.mockResolvedValue(mockResponse);
  ...
  describe('getDetails', () => {
    it('should get box details and map labels', async () => {
      ...
      const expectedResult: BoxGetResponse = {
        id: boxId,
        image: 'img1',
        status: 'running',
        labels: { a: 'b' },
  ...
  describe('create', () => {
    it('should create a box and map response labels', async () => {
      ...
      const expectedResult: BoxCreateResponse = {
        id: 'newBox',
        image: 'test-image',
        status: 'created',
        labels: { user: 'creator' },
  ...
  describe('deleteAll', () => {
    it('should delete all boxes', async () => {
      const mockData: BoxesDeleteResponse = {
        count: 2,
        ids: ['box1', 'box2'],
    ...
    it('should delete all boxes with force', async () => {
      const mockData: BoxesDeleteResponse = {
        count: 2,
        ids: ['box1', 'box2'],
        message: 'All boxes deleted forcefully',
      };
      const mockResponse: Partial<AxiosResponse> = { data: mockData };
      mockRequest.mockResolvedValue(mockResponse);
  ...
  describe('run', () => {
    ...
    it('should run command without box in response', async () => {
      ...
      const apiPayload = { cmd: command };
      const mockApiResponseData = {
        // API might not return box if not modified
        stdout: '.',
        stderr: '',
        exitCode: 0,
      };
  ...
  describe('reclaim', () => {
    it('should reclaim a specific box', async () => {
      const boxId = 'box123';
      const mockData: BoxReclaimResponse = {
        deletedIds: [boxId],
        message: 'Box reclaimed',
      };
      const mockResponse: Partial<AxiosResponse> = { data: mockData };
      mockRequest.mockResolvedValue(mockResponse);
      ...
      expect(mockRequest).toHaveBeenCalledWith({
        method: 'post',
        url: `/api/v1/boxes/${boxId}/reclaim`,
        data: { force: false },
        params: undefined,
        headers: undefined,
        responseType: 'json',
    ...
    it('should reclaim all inactive boxes with force', async () => {
      const mockData: BoxReclaimResponse = {
        deletedIds: ['box1', 'box2'],
        message: 'Inactive boxes reclaimed forcefully',
      };
      const mockResponse: Partial<AxiosResponse> = { data: mockData };
      mockRequest.mockResolvedValue(mockResponse);
  ...
  describe('extractArchive', () => {
    it('should extract an archive to a box', async () => {
      ...
      const mockData: BoxExtractArchiveResponse = {
  ...
  describe('error handling', () => {
    ...
    it('should throw APIError for other 4xx/5xx errors', async () => {
      ...
      await expect(boxApi.start(boxId)).rejects.toThrow(
      ...
      try {
        ...
        expect((e as APIError).responseData).toEqual({
    ...
    it('should throw GBoxError for non-Axios errors', async () => {
      ...
      await expect(boxApi.list()).rejects.toThrow(
        'An unexpected error occurred: Network fail'
      );
    });
  });
});
