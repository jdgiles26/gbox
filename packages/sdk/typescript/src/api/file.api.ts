...
export class FileApi extends Client {
   ...
   */
  async getStat(path: string): Promise<FileInfo | null> {
    const normalizedPath = this.normalizePath(path);
    try {
      const headers = await super.head(`${API_PREFIX}${normalizedPath}`);
      ...
      if (statHeader) {
        try {
          ...
          console.error(`[GBox SDK] Failed to parse x-gbox-file-stat header for path ${path}: ${e}`);
   ...
   */
  async getContent(path: string): Promise<ArrayBuffer> {
    const normalizedPath = this.normalizePath(path);
    // Use Client.getRaw
    return super.getRaw(`${API_PREFIX}${normalizedPath}`, undefined, { 'Accept': '*/*' });
  }
  ...
  async reclaim(): Promise<FileReclaimApiResponse> {
     ...
     // Use Client.post
     return super.post<FileReclaimApiResponse>(`${API_PREFIX}`, data);
  }
   ...
   */
  async share(boxId: string, path: string): Promise<FileShareApiResponse> {
    const normalizedPath = this.normalizePath(path);
    const data = { boxId: boxId, path: normalizedPath, operation: 'share' };
    // Use Client.post
    return super.post<FileShareApiResponse>(`${API_PREFIX}`, data);
  }

  // Removed getInfo, readContent, shareFromBox, exists
}

// No local axios function needed
